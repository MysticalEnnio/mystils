 ...
 */
function tFunc(parameterTypes, func, options) {
  options ??= [];
  ...
  return function (...args) {
    ...
    // definitions count
    if (parameterTypes.length !== args.length && options.includes("argLength"))
      throw new Error(
...
class mystils {
  ...
  isEven = tFunc([Number], (num) => num % 2 === 0);
  ...
  mergeSort = tFunc([Array], (array) => {
    ...
    return merge(mt.mergeSort(left), mt.mergeSort(array));
  });
  //https://stackoverflow.com/questions/43194336/javascript-counting-sort-implementation
  countingSort = tFunc([Array], (arr) => {
    arr
